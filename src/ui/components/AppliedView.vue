    ...
    <div v-else class="space-y-2.5">
      <div v-for="(item, idx) in applied" :key="idx" class="bg-slate-900/60 border border-slate-800/60 rounded-xl p-3 flex flex-col sm:flex-row sm:items-center justify-between gap-3 hover:border-slate-700 transition-colors">
        <div class="flex items-start space-x-3 truncate">
          <span v-if="item.mode === 'symlink'" class="text-base mt-0.5 shrink-0">🔗</span>
          <span v-else class="text-base mt-0.5 shrink-0">💉</span>
          <div class="truncate">
        ...
        <button class="bg-slate-950 hover:bg-rose-950/40 border border-slate-800 hover:border-rose-900 text-slate-400 hover:text-rose-300 px-2.5 py-1 rounded-lg text-xs transition-colors shrink-0 self-end sm:self-auto" @click="emit('remove', item)">
          解绑/卸载
        </button>
      </div>
    </div>
